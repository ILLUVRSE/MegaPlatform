...
5. Validate auth and realtime token issuance:
...
6. Promote the same rotated values to production after stage validation succeeds.
7. Remove the previous JWT secret and previous LiveKit key pair only after the overlap window has expired and validation is clean.
8. Update `ops/governance/key-rotation.json` with the new `lastRotatedAt` values and capture the completed ticket in ops notes.
