...
const payloadSchema = z.object({
  skeletonType: z.string().min(1),
  boneCount: z.number().int().positive(),
  constraints: z.array(z.object({ constraintId: z.string().min(1), type: z.string().min(1) }))
});
...
export async function POST(req: Request) {
  ...
  const result = await validateRigContractStandardization(parsed.data);
