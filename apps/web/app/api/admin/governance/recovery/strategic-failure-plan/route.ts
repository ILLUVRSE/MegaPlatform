...
const payloadSchema = z.object({
  failures: z.array(z.object({ id: z.string().min(1), severity: z.number().min(0).max(1), reversible: z.boolean() }))
});
...
export async function POST(req: Request) {
  ...
  const result = await planStrategicFailureRecovery(parsed.data);
