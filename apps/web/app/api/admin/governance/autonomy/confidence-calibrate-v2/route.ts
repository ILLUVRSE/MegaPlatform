...
const payloadSchema = z.object({
  outcomes: z.array(z.object({ confidence: z.number().min(0).max(1), realizedSuccess: z.number().min(0).max(1) }))
});
...
export async function POST(req: Request) {
  ...
  const result = await calibrateAutonomyConfidenceV2(parsed.data);
