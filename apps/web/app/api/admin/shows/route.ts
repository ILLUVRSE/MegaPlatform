...
/**
 * Shows collection API.
 * GET: ?q=&page= -> { data, page, totalPages }
 * POST: { title, slug, description?, posterUrl?, heroUrl? } -> { id }
...
import { apiInvalidPayload, apiUnauthorized } from "@/lib/apiError";
...
export async function GET(request: Request) {
  ...
  const { page, skip, take } = getPagination(searchParams.get("page"));
  const where = query
    ? {
        OR: [
  ...
  const shows = await prisma.show.findMany({
    ...
    skip,
    take,
    select: { id: true, title: true, slug: true, createdAt: true, isPremium: true, price: true }
  });
...
export async function POST(request: Request) {
  ...
  await writeAudit(auth.session.user.id, "shows:create", `Created show ${show.title}`);
