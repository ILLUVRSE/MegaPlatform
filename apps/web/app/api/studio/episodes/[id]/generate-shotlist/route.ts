...
import { generateDraftShotlistSuggestions } from "@/lib/showShotlistSuggestions";
...
export async function POST(
  ...
  try {
    const result = await generateDraftShotlistSuggestions(episode);
    return NextResponse.json(result);
  } catch (error) {
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Unable to generate draft shotlist" },
      { status: 409 }
    );
  }
}
